...
def get_source_pixels_in_aperture(ra, dec, radius, fitsobj):
    ...
    xr, yr = fitsobj.world2pix(ra, dec, 1)
...
def reproject_galactic(fitsimage, fitsobj):
    
    """ Reproject Galactic pointings. """

    
    print('Reprojecting ', fitsimage)
    image_data = fitsobj.data
    wcs_in = fitsobj.wcs
    
    # Create an optimal RA/Dec WCS
    ...
    # Reproject
    array_reproj, footprint = reproject_interp(
        (image_data, wcs_in),
        wcs_out,
        shape_out=shape_out
    )

    # Turn (ny,nx) into (1,1,ny,nx)
    array_reproj = array_reproj[np.newaxis, np.newaxis, :, :]
     
    # Build output header
    header_out = fitsobj.hdr.copy()
    ...
    fits.PrimaryHDU(
        data=array_reproj,
        header=header_out
    ).writeto(fitsimage, overwrite=True)
...
def measure_flux_density(fitsimage, rms, 
    ...
    #fitsobj.hdr['CRVAL1']#NAXIS1
    if fitsobj.hdr['CTYPE1']=='GLON-SIN':
        reproject_galactic(fitsimage, fitsobj)
        # read in reprojection
        fitsobj = fitsutils.Fits(fitsimage)
